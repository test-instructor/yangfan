...
> * 测试套件由`API`组成，复制接口管理中的`API`并做外键关联，方便后续的数据统计。
> * 测试套件可以复制多个相同或者不同的`API`，与`API`为多对多关系
