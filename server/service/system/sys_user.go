...
func (userService *UserService) Login(u *system.SysUser) (userInter *system.SysUser, err error) {
	...
	err = global.GVA_DB.Where("username = ?", u.Username).Preload("Projects").Preload("Authorities").Preload("Authority").First(&user).Error
	if err == nil {
...
func (userService *UserService) GetUserInfo(uuid uuid.UUID) (user system.SysUser, err error) {
	...
	err = global.GVA_DB.Preload("Projects").Preload("Authorities").Preload("Authority").First(&reqUser, "uuid = ?", uuid).Error
	if err != nil {
		return reqUser, err
	}
