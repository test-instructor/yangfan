...
func (p *RunnerTaskProducer) SendTask(taskID string, reportID uint, caseType string, caseID uint, runMode string, nodeName string, envID int, configID int, projectID uint) (string, error) {
	// 1. Node Selection
	targetNode := nodeName
	if targetNode == "" {
		var err error
