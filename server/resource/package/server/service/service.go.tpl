...
// Author [yourname](https://github.com/yourname)
func ({{.Abbreviation}}Service *{{.StructName}}Service)Delete{{.StructName}}(ctx context.Context, {{.PrimaryField.FieldJson}} string{{- if .AutoCreateResource -}},userID uint{{- end -}}, projectId int64) (err error) {
    var {{.Abbreviation}} {{.Package}}.{{.StructName}}
    err = {{$db}}.Where("{{.PrimaryField.ColumnName}} = ?", {{.PrimaryField.FieldName}}).First(&{{.Abbreviation}}).Error
    if err != nil {
        return err
    }
    if {{.Abbreviation}}.ProjectId != projectId {
...
// Author [yourname](https://github.com/yourname)
func ({{.Abbreviation}}Service *{{.StructName}}Service)Update{{.StructName}}(ctx context.Context, {{.Abbreviation}} {{.Package}}.{{.StructName}}, projectId int64) (err error) {
    var old{{.StructName}} {{.Package}}.{{.StructName}}
    err = {{$db}}.Model(&old{{.StructName}}).Where("{{.PrimaryField.ColumnName}} = ?", {{.Abbreviation}}.{{.PrimaryField.FieldName}}).First(&old{{.StructName}}).Error
    if err != nil {
        return err
    }
    if old{{.StructName}}.ProjectId != projectId {
        return errors.New("没有该项目的操作权限")
    }
...
func ({{.Abbreviation}}Service *{{.StructName}}Service)Get{{.StructName}}InfoList(ctx context.Context) (list []*{{.Package}}.{{.StructName}},err error) {
    // 创建db
	db := {{$db}}.Model(&{{.Package}}.{{.StructName}}{}).Order("id DESC")
