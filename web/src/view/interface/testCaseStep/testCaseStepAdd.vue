  ...
  Sortable.create(dom, {
    ...
    async onEnd(obj) {
      ...
      if (from === to) {
        ...
        // 复制接口到用例下
        const res = await addTestCase({ apiID: currRow.ID, caseID: Number(caseId) })
        if (res.code === 0) {
          ElMessage({
            type: 'success',
            message: '添加套件成功'
...
const getTableData = async() => {
  const table = await getInterfaceTemplateList({ type: apiTypes,menu: treeID, page: 1, pageSize: 99999})
  if (table.code === 0) {
