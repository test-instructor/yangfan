  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="用户项目" min-width="150">
          <template #default="scope">
            <el-cascader
                v-model="scope.row.projectIds"
                :clearable="false"
                :options="projectOptions"
                :props="{ multiple:true,checkStrictly: true,label:'projectName',value:'projectId',disabled:'disabled',emitPath:false}"
                :show-all-levels="false"
                collapse-tags
                @visible-change="(flag)=>{changeProject(scope.row,flag)}"
                @remove-tag="()=>{changeProject(scope.row,false)}"
            />
          </template>
        </el-table-column>
    ...
    >
      <div style="height:60vh;overflow:auto;padding:0 12px;">
        <el-form ref="userForm" :rules="rules" :model="userInfo" label-width="80px">
          ...
          <el-form-item label="用户项目" prop="projectId">
            <el-cascader
...
import { getAuthorityList, getProjectList } from '@/api/authority'
...
const setProjectOptions = (ProjectData, optionsData) => {
  ProjectData &&
...
const initPage = async() => {
  ...
  const res1 = await getProjectList({page: 1, pageSize: 999})
  setOptionsProject(res1.data.list)
}
...
const setProjectIds = () => {
  tableData.value && tableData.value.forEach((user) => {
    const projectIds = user.projects && user.projects.map(i => {
...
const setOptionsProject = (projectData) => {
  projectOptions.value = []
  setProjectOptions(projectData, projectOptions.value)
}
...
const rules = ref({
  ...
  projectId: [
    {required: true, message: '请选择用户项目', trigger: 'blur'}
  ]
})
const userForm = ref(null)
const enterAddUserDialog = async() => {
  userInfo.value.authorityId = userInfo.value.authorityIds[0]
  userInfo.value.projectId = userInfo.value.projectIds[0]
...
const closeAddUserDialog = () => {
  ...
  userInfo.value.projectId = ''
  userInfo.value.projectIds = []
...
const changeProject = async (row, flag) => {
  if (flag) {
    return
  }

  await nextTick()
  const res = await setUserProjects({
    ID: row.ID,
