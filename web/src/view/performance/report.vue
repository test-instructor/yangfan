      ...
      >
        ...
        <el-table-column align="left" label="执行状态" width="120">
          <template #default="scope">
            <a-tag :key="runState(scope.row.state)[1]" :color="runState(scope.row.state)[1]">{{ runState(scope.row.state)[0] }}</a-tag>
          </template>
        </el-table-column>

        <el-table-column align="left" label="按钮组">
          <template #default="scope">
                  <el-button v-if="runState(scope.row.state)[2]" type="text" icon="document" size="small" class="table-button" @click="reportDetailFunc(scope.row)">详情</el-button>
                  <el-button v-if="!runState(scope.row.state)[2]" type="text" icon="document" size="small" class="table-button" @click="reportDetailFunc(scope.row)" disabled>详情</el-button>
