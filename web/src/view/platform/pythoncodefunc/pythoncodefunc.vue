  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="项目信息" prop="projectId" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updatePythonCodeFuncFunc(scope.row)">编辑</el-button>
    ...
    <ArrayCtrl v-model="detailForm.params"/>
</el-descriptions-item>
                    <el-descriptions-item label="完整代码">
    {{ detailForm.fullCode }}
</el-descriptions-item>
                    <el-descriptions-item label="起始索引">
    {{ detailForm.startIndex }}
</el-descriptions-item>
...
import {
  ...
  deletePythonCodeFunc,
  deletePythonCodeFuncByIds,
  updatePythonCodeFunc,
  findPythonCodeFunc,
  getPythonCodeFuncList
...
const getTableData = async() => {
  const table = await getPythonCodeFuncList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deletePythonCodeFuncByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePythonCodeFuncFunc = async(row) => {
    const res = await findPythonCodeFunc({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePythonCodeFuncFunc = async (row) => {
    const res = await deletePythonCodeFunc({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
