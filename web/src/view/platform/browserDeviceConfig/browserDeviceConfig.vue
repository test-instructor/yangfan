  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="浏览器标识" prop="browserId" width="120" />

            <el-table-column align="left" label="登录状态" prop="logOn" width="120">
    ...
    <template #default="scope">{{ formatBoolean(scope.row.ignorePopup) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateBrowserDeviceOptionsFunc(scope.row)">编辑</el-button>
...
import {
  ...
  deleteBrowserDeviceOptions,
  deleteBrowserDeviceOptionsByIds,
  updateBrowserDeviceOptions,
  findBrowserDeviceOptions,
  getBrowserDeviceOptionsList
...
const getTableData = async() => {
  const table = await getBrowserDeviceOptionsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBrowserDeviceOptionsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBrowserDeviceOptionsByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBrowserDeviceOptionsFunc = async(row) => {
    const res = await findBrowserDeviceOptions({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBrowserDeviceOptionsFunc = async (row) => {
    const res = await deleteBrowserDeviceOptions({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
