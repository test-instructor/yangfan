<template>
  <el-dialog v-model="visible" title="环境管理" width="700px">
    ...
    <el-table :data="table" style="width: 100%">
      ...
      <el-table-column label="备注" prop="remarks">
        <template #default="scope">
          <template v-if="scope.row._editing || scope.row._isNewRow">
            <el-input v-model="scope.row._remarksEdit" placeholder="请输入备注" />
          </template>
          <template v-else>
            {{ scope.row.remarks }}
          </template>
...
import { createEnv, updateEnv, deleteEnv, getEnvList } from '@/api/platform/env'
...
const loadTable = async () => {
  const res = await getEnvList({ page: page.value, pageSize: pageSize.value })
  if (res.code === 0) {
...
const cancelEdit = (row) => { row._editing = false; row._nameEdit = row.name; row._remarksEdit = row.remarks }
const saveRow = async (row) => {
  const payload = { ID: row.ID, name: row._nameEdit, remarks: row._remarksEdit }
  if (!payload.name || payload.name.trim() === '') { ElMessage({ type: 'warning', message: '名称为必填' }); return }
  const res = await updateEnv(payload)
  if (res.code === 0) {
    row.name = row._nameEdit
    row.remarks = row._remarksEdit
    row._editing = false
...
const deleteRow = async (row) => {
  ElMessageBox.confirm('确定要删除该环境吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    .then(async () => {
...
const startCreateRow = () => {
  ...
  if (hasNew) return
  table.value.push({ _isNewRow: true, _nameEdit: '', _remarksEdit: '', _editing: true })
}
const cancelNewRow = () => { table.value = table.value.filter(r => !r._isNewRow) }
const saveNewRow = async (row) => {
  const payload = { name: row._nameEdit, remarks: row._remarksEdit }
  if (!payload.name || payload.name.trim() === '') { ElMessage({ type: 'warning', message: '名称为必填' }); return }
  const res = await createEnv(payload)
  if (res.code === 0) {
