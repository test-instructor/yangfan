...
func LoadTestCases(tests ...ITestCase) ([]*TestCase, error) {
	...
	for _, iTestCase := range tests {
		...
		// Handle TestCasePath (file path or folder path)
		if tcPath, ok := iTestCase.(*TestCasePath); ok {
		...
		// For any other ITestCase implementation, call GetTestCase() interface method
		tc, err := iTestCase.GetTestCase()
		if err != nil {
			return nil, errors.Wrap(err, "failed to get testcase from ITestCase")
		}
		if tc != nil {
			testCases = append(testCases, tc)
		}
