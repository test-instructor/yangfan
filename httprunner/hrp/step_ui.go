...
func runStepMobileUI(s *SessionRunner, step IStep) (stepResult *StepResult, err error) {
	...
	for _, action := range mobileStep.Actions {
		...
		default:
			...
			// Execute action with cancellable context and action-level timeout.
			//
			// Why:
			// - Some device operations may block for a long time.
			// - Testcase timeout/interrupt should be able to cancel the current action.
			// - Action-level timeout prevents waiting until testcase timeout.
			ctx, cancelCause := context.WithCancelCause(context.Background())
			defer cancelCause(nil)

			actionOpts := option.NewActionOptions(action.GetOptions()...)
			actionTimeout := 10 * time.Second
			if actionOpts.Timeout > 0 {
				actionTimeout = time.Duration(actionOpts.Timeout) * time.Second
			}

			// Add action timeout that also records the cause.
			actionTimer := time.NewTimer(actionTimeout)
			defer func() {
				if !actionTimer.Stop() {
					select {
					case <-actionTimer.C:
			...
			go func() {
				...
				case <-s.caseRunner.hrpRunner.interruptSignal:
					...
					cancelCause(code.InterruptError)
				case <-s.caseRunner.hrpRunner.caseTimeoutTimer.C:
					log.Warn().Msg("cancelling action due to case timeout")
					cancelCause(code.TimeoutError)
				case <-actionTimer.C:
					log.Warn().Msg("cancelling action due to action timeout")
					cancelCause(code.TimeoutError)
				case <-ctx.Done():
