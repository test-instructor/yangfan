...
type Request struct {
	...
	DataWarehouse  map[string]interface{} `json:"data_warehouse,omitempty" yaml:"data_warehouse,omitempty"`
}
...
func queryDataWarehouse(dataWarehouse map[string]interface{}, stepVariables map[string]interface{}) ([]map[string]interface{}, error) {
	if dataWarehouse == nil || len(dataWarehouse) == 0 {
		return nil, nil
	}

	// Get platformProjectId and platformEnvId from variables
	projectID, hasProjectID := stepVariables["platformProjectId"]
	envID, hasEnvID := stepVariables["platformEnvId"]

	if !hasProjectID || !hasEnvID {
		log.Warn().Msg("platformProjectId or platformEnvId not found in variables, skipping data warehouse query")
		return nil, nil
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to marshal data warehouse request body")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to create data warehouse request")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to query data warehouse")
	}
	defer resp.Body.Close()
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to read data warehouse response")
	}

	// Parse response
	var dwResp DataWarehouseResponse
	if err := json.Unmarshal(respBody, &dwResp); err != nil {
		return nil, errors.Wrap(err, "failed to unmarshal data warehouse response")
	}

	if dwResp.Code != 0 && dwResp.Code != 200 {
		return nil, errors.Errorf("data warehouse API returned error: %s (code: %d)", dwResp.Msg, dwResp.Code)
	}

	// Check if count matches total
	if dwResp.Count != dwResp.Total {
		log.Warn().
			Int("count", dwResp.Count).
			Int("total", dwResp.Total).
			Msg("data warehouse returned partial data: count does not match total")
	}

	log.Info().
		Int("count", dwResp.Count).
		Int("total", dwResp.Total).
		Int("list_size", len(dwResp.List)).
		Msg("data warehouse query successful")
	return dwResp.List, nil
}

// processDataWarehouseData processes data warehouse response with multiple rows and converts to parameters
func processDataWarehouseData(data []map[string]interface{}, stepRequest *StepRequestWithOptionalArgs) error {
	if len(data) == 0 {
		log.Warn().Msg("data warehouse returned empty data")
		return nil
	}
	...
	// Note: In Go, map iteration order is random, so we sort to ensure deterministic key
	...
	// Set parameters in the format expected by httprunner
	...
	log.Info().
		...
		Int("rows", len(values)).
		Msg("data warehouse data converted to parameters")
