...
import (
	...
	"github.com/test-instructor/yangfan/httprunner/internal/version"
	"github.com/test-instructor/yangfan/httprunner/mcphost"
...
// StepCompleteCallback is the callback function type for step completion events.
// stepType: the type of the completed step (e.g., "request", "api", "testcase")
// stepResult: the result of the completed step
type StepCompleteCallback func(stepType string, stepResult *StepResult)
...
// SetOnStepComplete sets the callback function that will be called after each step completes.
func (r *HRPRunner) SetOnStepComplete(callback StepCompleteCallback) *HRPRunner {
	r.onStepComplete = callback
	return r
}
...
// e.g. {"foo": "bar", "age": 18} -> "foo:bar_age:18"
// If the result exceeds 50 characters, it will be truncated with "..." appended.
func formatParameters(params map[string]interface{}) string {
	...
	result := strings.Join(pairs, "_")

	// truncate if longer than 50 characters (use rune slice to handle multi-byte characters correctly)
	runes := []rune(result)
	if len(runes) > 50 {
	...
	for loopIndex := 1; loopIndex <= loopTimes; loopIndex++ {
		for _, params := range allParameters {
			...
			if hasParameters {
				...
				if hasLoops {
					currentStepName = fmt.Sprintf("%s [loop_%d_params-%s]", stepName, loopIndex, paramStr)
				} else {
					currentStepName = fmt.Sprintf("%s [params-%s]", stepName, paramStr)
				}
			} else if hasLoops {
...
func (r *SessionRunner) RunStep(step IStep) (stepResult *StepResult, err error) {
	...
	// query data warehouse if step is a request with DataWarehouse config
	if err := r.queryAndApplyDataWarehouse(step); err != nil {
		log.Error().Err(err).Msg("query data warehouse failed")
		if r.caseRunner.hrpRunner.failfast {
			return nil, errors.Wrap(err, "query data warehouse failed")
		}
	}
...
func (r *SessionRunner) executeStepWithVariables(step IStep, stepName string, parameters map[string]interface{}) (stepResult *StepResult, err error) {
	...
	originalName := stepConfig.StepName
	stepConfig.StepName = stepName
	...
	// call onStepComplete callback if set
	if r.caseRunner.hrpRunner.onStepComplete != nil {
		r.caseRunner.hrpRunner.onStepComplete(stepType, stepResult)
	}

	// restore original variables to avoid side effects
	stepConfig.Variables = originalVariables
	// restore original name
	stepConfig.StepName = originalName
...
// queryAndApplyDataWarehouse queries data warehouse and applies the result to step config
func (r *SessionRunner) queryAndApplyDataWarehouse(step IStep) error {
	// check if step is a request step
	stepRequest, ok := step.(*StepRequestWithOptionalArgs)
	if !ok {
		// not a request step, skip
		return nil
	}

	// check if request has DataWarehouse config
	if stepRequest.Request == nil || stepRequest.Request.DataWarehouse == nil || len(stepRequest.Request.DataWarehouse) == 0 {
		// no data warehouse config, skip
		return nil
	}

	log.Info().Msg("querying data warehouse for step")

	// merge session variables with step variables for query
	queryVariables := mergeVariables(r.sessionVariables, stepRequest.Variables)

	// query data warehouse
	dwData, err := queryDataWarehouse(stepRequest.Request.DataWarehouse, queryVariables)
	if err != nil {
		return errors.Wrap(err, "query data warehouse failed")
	}

	if dwData == nil || len(dwData) == 0 {
		log.Warn().Msg("data warehouse returned no data")
		return nil
	}

	if len(dwData) == 1 {
		// Single row: populate step variables directly
		log.Info().Msg("data warehouse returned 1 row, populating step variables")
		for key, value := range dwData[0] {
			stepRequest.Variables[key] = value
		}
	} else {
		// Multiple rows: convert to Parameters format for parameterized execution
		log.Info().Int("count", len(dwData)).Msg("data warehouse returned multiple rows, converting to parameters")

		// Extract all column names from the first row
		var columnNames []string
		for key := range dwData[0] {
			columnNames = append(columnNames, key)
		}

		// Sort column names for consistency
		if len(columnNames) == 0 {
			return errors.New("data warehouse returned data with no columns")
		}
		...
		// Set parameters in step config
		if stepRequest.Parameters == nil {
			stepRequest.Parameters = make(map[string]interface{})
		}
		stepRequest.Parameters[combinedKey] = values

		log.Info().
			Str("parameter_key", combinedKey).
			Int("rows", len(values)).
			Msg("data warehouse data applied to step parameters")
	}
