...
func (t *ToolGetForegroundApp) Implement() server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		// NOTE: This tool previously relied on ForegroundInfo to query the
		// current foreground app. That logic has been disabled per requirement
		// to remove all ForegroundInfo usages. The original implementation is
		// kept below as a comment for reference.
		//
		// driverExt, err := setupXTDriver(ctx, request.GetArguments())
		// if err != nil {
		// 	return nil, fmt.Errorf("setup driver failed: %w", err)
		...
		return NewMCPErrorResponse("GetForegroundApp is disabled because ForegroundInfo is no longer available"), nil
	}
}
