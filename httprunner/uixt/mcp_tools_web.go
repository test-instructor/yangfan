...
func (t *ToolTapBySelector) Implement() server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		// Build all options from request arguments
		// Important: propagate handler context into driver options so action-level
		// timeouts/interrupts can cancel lower-level operations (e.g. adb uiautomator dump).
		opts := append(unifiedReq.Options(), option.WithContext(ctx))
