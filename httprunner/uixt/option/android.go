...
import "github.com/test-instructor/yangfan/httprunner/pkg/gadb"
...
type AndroidDeviceOptions struct {
	...
	// composite driver - enables per-operation driver switching between ADB and UIA2
...
func (dev *AndroidDeviceOptions) Options() (deviceOptions []AndroidDeviceOption) {
	...
	if dev.Composite {
		deviceOptions = append(deviceOptions, WithComposite(true))
	}
	if dev.UIA2 {
...
// WithComposite enables CompositeDriver for per-operation driver switching
// When enabled, each action can specify whether to use ADB or UIA2 driver
func WithComposite(composite bool) AndroidDeviceOption {
	return func(device *AndroidDeviceOptions) {
		device.Composite = composite
	}
}
