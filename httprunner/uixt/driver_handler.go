...
func callMCPActionTool(driver IDriver,
	...
	// If MCP execution/clients are not configured, skip early with a clear log to
	// avoid misleading warnings such as “server not found”.
	if len(dExt.loadedMCPClients) == 0 {
		log.Info().
			Str("action", actionType).
			Str("server", serverName).
			Bool("use_mcp_execution", UseMCPExecution()).
			Msg("skip MCP action tool: no MCP clients loaded (MCP likely disabled)")
		return
	}
...
func getCommonMCPArguments(driver IDriver) map[string]interface{} {
	...
	// Keeping the original logic here as a comment for future reference.
	//
